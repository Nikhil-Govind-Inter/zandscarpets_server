...
function loadRoutes(dirPath, baseRoute = "", depth = 0) {
  ...
  try {
    ...
    console.error(
      `${indent}${colors.red(`✗ Cannot read ${dirPath} — ${error.message}`)}`,
    );
    errorCount++;
    return router;
  }
      ...
      try {
        ...
        if (
          ...
          if (routeMap.has(fullRoute)) {
            warnings.push(
              `Duplicate route "${fullRoute}" (${file} conflicts with ${routeMap.get(fullRoute)})`,
            );
            console.warn(
              `${indent}${colors.yellow(`⚠ ${fullRoute} — duplicate of ${routeMap.get(fullRoute)}`)}`,
            );
          } else {
            routeMap.set(fullRoute, file);
            const slowFlag =
              loadTime > 50 ? colors.yellow(` (slow: ${loadTime}ms)`) : "";
            console.log(
              `${indent}${colors.green(`✓ ${fullRoute}`)}${slowFlag}`,
            );
          }
          ...
          console.error(
            `${indent}${colors.red(`✗ ${fullRoute} — ${reason} [${file}]`)}`,
          );
          errorCount++;
        }
      } catch (error) {
        ...
        console.error(
          `${indent}${colors.red(`✗ ${fullRoute} — ${error.message} [${file}]`)}`,
        );
        errorCount++;
      }
    }
